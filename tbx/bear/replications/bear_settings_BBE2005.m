function s = bear_settings_BBE2005
...
% APPROACH One-Step Bayesian estimation (Gibbs Sampling) with three factors
% and FFR
...
s = BEARsettings(2, 'ExcelPath', fullfile(bearroot(), 'replications','data_BBE2005.xlsx') );
...
s.startdate='1959m2'; %(first differnces)
...
s.favar.transformation=0; % (1=yes, 0=no) // 'factor data' must contain values for startdate -1 in the case we have First Difference (2,5) transformation types and startdate -2 in the case we have Second Difference (3,6) transformation types
s.favar.transform_endo=''; %transformation codes of varendo variables other than factors (ordering follows 'data' sheet!)
...
s.favar.plotX='IP PUNEW FYGM3 FYGT5 FMFBA FM2 EXRJAN PMCP IPXMCA GMCQ GMCDQ GMCNQ LHUR PMEMP LEHCC HSFR PMNO FSDXP HHSNTN';

% choose shock(s) to plot
s.favar.plotXshock='FYFF';
...
s.lambda1=1; % rather diffuse prior
