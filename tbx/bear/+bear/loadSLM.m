function dataSLM = loadSLM(datesendo,pref)

dataSLM = pref.data.SurveyLocalMean;
dataSLM = retime(dataSLM, datesendo, 'fillwithmissing');

% T=size(data_endo,1)-p;
% %first check for which variables the survey local mean is available and the read the local mean data
% [dataSLM, namesSLM, rawSLM]=xlsread(pref.data.InputFile,'Survey Local Mean');
