...
ybarT=bear.vec(flipud(Y(finalp-p+1:finalp,:))');
...
[fmat ortirfcell]=bear.tvcfsim1(beta_iter,D,ybarT,data_exo_p,Fperiods,n,m,p,k);
...
% obtain the vector of shocks generating the conditions, depending on the type of conditional forecasts selected by the user
   ...
   if CFt==1
   eta=bear.shocksim5(cfconds,Fperiods,n,fmat,ortirfcell);
   % if instead the user selected the shock-specific setting
   elseif CFt==2
   eta=bear.shocksim6(cfconds,cfshocks,cfblocks,Fperiods,n,gamma,fmat,ortirfcell);
   end
