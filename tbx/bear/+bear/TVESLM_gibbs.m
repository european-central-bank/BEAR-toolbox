...
hbar = bear.parfor_progressbar(It,'Progress of Gibbs Sampler');  %create the progress bar
...
for isim = 1:It
    ...
    PsiDraw_prop = bear.samplePsi_DK(YData,YsData, Bdraw,Hdraw,Vdraw,Gdraw,CDdraw,meanTS,priorValues.kappa,dataValues,KFSmatrices);
   ...
   [Gdraw_prop,phi_Gdraw_prop] = bear.sampleG(zData_prop,PsiDraw_prop,CDdraw,phi_Gdraw,Gdraw,priorValues,dataValues);
    % sample the stochastic volatility components of the VAR residuals   
   %[Hdraw_prop,HvarsDraw_prop,phi_Hdraw_prop,Adraw_prop] =bear.sampleH(YData,PsiDraw_prop,Adraw,Bdraw,phi_Hdraw,HvarsDraw,priorValues,dataValues);
   [Hdraw_prop,HvarsDraw_prop,phi_Hdraw_prop,Adraw_prop,h0,Kh0]=bear.sampleH_update(YData,PsiDraw_prop,Adraw,Bdraw,phi_Hdraw,HvarsDraw,priorValues,dataValues,h0,Kh0);
   ...
   % sample the stochastic volatility components of the transition equation for the local mean    
    [Vdraw_prop,phi_Vdraw_prop] = bear.sampleV(PsiDraw_prop,phi_Vdraw,Vdraw,priorValues,dataValues);
    % sample the VAR coefficients
   [Bdraw_prop,stationary]=bear.sampleBinw(YData,PsiDraw_prop,Hdraw_prop,p,priorValues, q, k);
    end
    ...
    if isim > Bu && mod( isim - Bu, simstep ) == 0
      ...
      beta_gibbs(:,isave)=bear.vec(Bdraw);
      ...
      sigma_gibbs(:,isave)=bear.vec(Hdraw(:,:,T));
         for jj=1:T
