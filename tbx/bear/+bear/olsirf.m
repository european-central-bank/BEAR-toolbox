...
gamma=sigmahat;
elseif IRFt==2
D=chol(bear.nspd(sigmahat),'lower');
gamma=eye(n);
elseif IRFt==3
[D,gamma]=bear.triangf(sigmahat);
end
...
beta=betahat+chol(bear.nspd(omegahat),'lower')*randn(q,1);
...
% write in excel
if pref.results==1
    bear.xlswritegeneral(fullfile(pref.results_path, [pref.results_sub '.xlsx']),strshockcell,'strctshocks','B2');
end
