function [beta0, omega0] = ogrmaprior(ar, arvar, lambda1, lambda2, lambda3, ...
    lambda5, n, p, k1, q1, bex, blockexo)
...
for ii = 1:n
   for jj = 1:p
       omega0((ii - 1)*k1 + (jj - 1)*n + ii, (ii - 1)*k1 + (jj - 1)*n + ii) = ...
           (lambda1/jj^lambda3)^2;
   end
end
...
for ii = 1:n
   for jj = 1:p
      for kk = 1:n
          if kk ~= ii
              omega0((ii - 1)*k1 + (jj - 1)*n + kk, (ii - 1)*k1 + (jj - 1)*n + kk) = ...
                  (arvar(ii, 1)/arvar(kk, 1)) * (((lambda1 * lambda2)/(jj^lambda3))^2);
          end
      end
   end
end
...
if bex == 1
   for ii = 1:n
      for jj = 1:n
         if blockexo(ii,jj) == 1
            for kk = 1:p
                omega0((jj - 1)*k1 + (kk - 1)*n + ii, (jj - 1)*k1 + (kk - 1)*n + ii) = ...
                    omega0((jj - 1)*k1 + (kk - 1)*n + ii, (jj - 1)*k1 + (kk - 1)*n + ii)*lambda5^2;
            end
         else
         end
      end
