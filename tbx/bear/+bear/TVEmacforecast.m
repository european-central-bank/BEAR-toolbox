function [cforecast_record]=TVEmacforecast(data_endo_a,data_exo_a,data_exo_p,It,Bu,Fperiods,cfconds,cfshocks,cfblocks,CFt,n,m,p,k1,k3,beta_gibbs,D_record,gamma_record,theta_gibbs,TVEHfuture,ss_record,indH)
...
%tempeq=[ss_record{1,1}(ii,end-1:end)' ss_record{2,1}(ii,end-1:end)' ss_record{3,1}(ii,end-1:end)'];
for j=1:n
...
fmat=bear.TVEmaforecastsim(data_endo_a,B,p,n,Fperiods,theta,TVEHfuture,tempeq,indH);


% step 3: compute IRFs and orthogonalised IRFs matrices
[~,ortirfmat]=bear.mairfsim(B,D,p,n,Fperiods);
