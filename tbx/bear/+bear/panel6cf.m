...
ybarT=bear.vec(flipud(Ymat(finalp-p+1:finalp,:))');
...
[fmat ortirfcell]=bear.panel6cfsim(theta_iter,D_iter,Xi,ybarT,data_exo_p,Fperiods,N,n,m,p,k);
...
% obtain the vector of shocks generating the conditions, depending on the type of conditional forecasts selected by the user
   ...
   if CFt==1
   eta=bear.shocksim3(cfconds,Fperiods,N,n,fmat,ortirfcell);
   % if instead the user selected the shock-specific setting
   elseif CFt==2
   eta=bear.shocksim4(cfconds,cfshocks,cfblocks,Fperiods,N,n,gamma_iter,fmat,ortirfcell);
   end
