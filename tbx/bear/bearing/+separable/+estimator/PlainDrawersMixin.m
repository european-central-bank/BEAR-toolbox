    ...
    methods
        function createDrawers(this, meta)
            ...
            function draw = conditionalDrawer(sample, startingIndex, forecastHorizon)
                draw = struct();
                % BEAR6-FIX: was wrap(sample.beta, ...) which leaves units in dim 2
                % (sample.beta is [q × N] for ZellnerHong / Hierarchical panels).
                % Sibling panel drawers (MeanOLSPanel, NormalWishartPanel) put units
                % in dim 3 via repmat(..., 1, 1, numCountries), and
                % base.@Structural.conditionalForecast extracts along dim 3
                % (EXTRACT_DIM=3 via system.extractUnitFromCellArray). Align by
                % reshaping sample.beta to [q × 1 × N] so units live in dim 3.
                betaPerUnit = reshape(sample.beta, size(sample.beta, 1), 1, []);
                draw.beta = wrap(betaPerUnit, forecastHorizon);
            end%
