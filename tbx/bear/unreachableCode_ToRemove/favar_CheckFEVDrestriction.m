function[success]=favar_CheckFEVDrestriction(ortirfmatrix,gamma,IRFperiods,n,rowrelativeFEVD,clmrelativeFEVD,rowabsoluteFEVD,clmabsoluteFEVD,FEVDresperiods,rowsFEVD,nFEVDresX,LFEVD)

%% Phase 1: Compute FEVD
% other than in the structual FEVD estimate plot afterwards, here the FEVD
% estimates are NOT adjusted to R2, i.e. the actual variance with the
% residual. The absolute and relative restrictions here must be interpreted
% with respect to the total variance that can be explained by all the endogenous
% variables (factors and other variables). E.g. an absolute  restriction
% for a shock would be fullfilled  when >50% are explained by this shocks
% within the the total variance that can be explained by all the shocks,
% but the *residual* variance (R2-adjusted) is excluded in this
% calculation.
