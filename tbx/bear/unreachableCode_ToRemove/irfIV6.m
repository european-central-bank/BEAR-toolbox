...
[EPSIV,IVcut,EPSt,sigmahatIV,B,EPS,df,sigma_hat,inv_sigma_hat,beginInstrument,EndInstrument,IV,txt,OverlapIVinY,cut1,cut2,cut3,cut4]=bear.loadIV(betahat,k,n,Y,X,T,p,names,startdate,enddate,strctident,pref);
...
%             IVrotate = IVrotate(1:length(IV),:);
        ...
        stationary=1;
        elseif IRFt==6
        [stationary]=bear.checkstable(betadraw,n,p,k); %switches stationary to 0, if the draw is not stationary
        end
end
        if IRFt==5
    ...
    [~,ortirfmatrix]=bear.irfsim(betadraw,Ddraw,n,m,p,k,IRFperiods);
       for jj=1:IRFperiods
