function [irf_record,D_record,gamma_record,hd_record,ETA_record,beta_record,sigma_record]=irfres61(beta_gibbs,sigma_gibbs,C_draws,IV_draws,IRFperiods,n,m,p,k,T,Y,X,signreslabels,FEVDresperiods,data_exo,HD,const,exo,strctident,pref,favar,IRFt,It,Bu,prior,theta_gibbs,Z,k3)
...
for ii=1:Acc %parfor
    ...
    if IRFt==4 && zerores==1 && signres==0 && magnres==0 && relmagnres==0 && favar_signres==0 && favar_magnres==0
        ...
        [~,ortirfmatrix]=bear.irfsim(beta,hsigma,n,m,p,k,max(IRFperiods,max(periods)),prior);
        ...
        while success==0
            ...
            elseif IRFt==6
                ...
                % then draw a random set of beta and sigma corresponding to this index (this is done to make it possible to draw, if required, an infinite number of values from the gibbs sampler record, with equal probability on each value)
                beta=beta_gibbs(:,index);
                %[stationary]=bear.checkstable(beta,n,p,k); %switches stationary to 0, if the draw is not stationary
                %end
            ...
            while success==1 && jj<=n && sum(okay)<n
                ...
                if IRFt==4 && zerores==1 || magnres==1 %if we have zero restrictions, we cannot reshuffle the columns
                    ...
                    if success==1 && favar_signres==1 | favar_magnres==1
                        ...
                        if favar_signres==1
                            [success]=bear.checksignres_favar(favar_Scell{1,jj},favar_fj);
                        end
                        % if 'success' is still equal to 1, also check for magnitude restrictions
                        if success==1 && favar_magnres==1
                            [success]=bear.checkmagres(favar_Mcell{1,jj},favar_Mlcell{1,jj},favar_Mucell{1,jj},favar_fj);
                        end
                    end
                    ...
                    if okay(1,1)==0 %first find the first column
                        ...
                        if success==1 && magnres==1
                            [success]=bear.checkmagres(Mcell{1,yy},Mlcell{1,yy},Mucell{1,yy},fj);
                        end
                        ...
                        if success==1 && favar_signres==1 | favar_magnres==1
                            ...
                            % simple check
                            if favar_signres==1
                                [success]=bear.checksignres_favar(favar_Scell{1,yy},favar_fj);
                            end
                            % if 'success' is still equal to 1, also check for magnitude restrictions
                            if success==1 && favar_magnres==1
                                [success]=bear.checkmagres(favar_Mcell{1,yy},favar_Mlcell{1,yy},favar_Mucell{1,yy},favar_fj);
                            end
                        end
                        if success==1
            ...
            if size(Qj,2)==n && success==1  && checkCorrelInstrumentShock==1
                ...
                ETA=D\EPS';
                [success,corIV,pivShockwithIV,Qj]=bear.CheckCorrelWithIV(ETA,n,CorrelShock_index, IVcorrel,OverlapIVcorrelinY,FlipCorrel,Qj);
            end
            
            %% check relative magnitude restrictions
            if size(Qj,2)==n && success==1  && relmagnres==1 | favar_relmagnres==1
                D=hsigma*Qj;
                [~,ortirfmatrixmagnitude]=bear.irfsim(beta,D,n,m,p,k,max(IRFperiods,max(mperiods)),prior);
                    ...
                    [success]=bear.checkrelmag(stackedirfmatmagn,columnsS,columnsW,rowsS,rowsW,n,mperiods,n);
                    end
                ...
                if success==1 && favar_relmagnres==1
                    ...
                    % check
                    [success]=bear.checkrelmag(favar_stackedirfmatmagn,favar_columnsS,favar_columnsW,favar_rowsS,favar_rowsW,n,favar_mperiods,nrelmagnresX);
                end
            end
            ...
            if size(Qj,2)==n && success==1  && FEVDres==1 | favar_FEVDres==1
                ...
                [~,ortirfmatrixFEVD]=bear.irfsim(beta,D,n,m,p,k,IRFperiods,prior); %obtain orthogonalized IRFs
                gamma=eye(n,n);
                %check FEVDrestrictions
                if FEVDres==1
                    [success]=bear.CheckFEVDrestriction(ortirfmatrixFEVD,gamma,IRFperiods,n,rowrelativeFEVD,clmrelativeFEVD,rowabsoluteFEVD,clmabsoluteFEVD,FEVDresperiods,rowsFEVD,n);
                end
                ...
                if success==1 && favar_FEVDres==1
                    ...
                    % check
                    [success]=bear.CheckFEVDrestriction_favar(favar_ortirfmatrixFEVD,gamma,IRFperiods,n,favar_rowrelativeFEVD,favar_clmrelativeFEVD,favar_rowabsoluteFEVD,favar_clmabsoluteFEVD,favar_FEVDresperiods,favar_rowsFEVD,nFEVDresX,LFEVD);
                end
            end
            ...
            if size(Qj,2)==n && success==1 && HD==1
                D=hsigma*Qj;
                [hd_estimates]=bear.hd_new_for_signres(const,exo,beta,k,n,p,D,m,T,X,Y,data_exo,IRFt,signreslabels_shocks,prior,theta,Z,k3);
            end
