function [beta_gibbs F_gibbs gamma_gibbs L_gibbs phi_gibbs sigma_gibbs lambda_t_gibbs sigma_t_gibbs sbar]=stvol_gibbs(stvol,Xbart,yt,beta0,omega0,alpha0,delta0,f0,upsilon0,betahat,sigmahat,I_o,omega,T,n,q,It,Bu,pick,pickf,varargin)
...
% draw the parameters in turn
   ...
   % estimate deltabar
   if stvol == 2
      delta_G = G{jj,1}'*I_o*G{jj,1};
   else
      delta_G = GIG;
   end
   deltabar=L(:,jj)'*delta_G*L(:,jj)+delta0;
...
% step 8: draw the series lambda_i,t from their conditional posteriors, i=1,...,n and t=1,...,T
   ...
   for jj=1:n
      ...
      % the definitions of lambdabar and phibar varies with the period, thus define them first
         ...
         if kk==1
         lambdabar=(g*L(2,jj))/(1/omega+g^2);
         phibar=phi(1,jj)/(1/omega+g^2);
         % if the period is the final period
         elseif kk==T
         lambdabar=g*L(T-1,jj);
         phibar=phi(1,jj);
         % if the period is any period in-between
         else
         lambdabar=(g/(1+g^2))*(L(kk-1,jj)+L(kk+1,jj));
         phibar=phi(1,jj)/(1+g^2);
         end
