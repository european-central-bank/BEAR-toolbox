...
[condDataTbl, planTbl] = tablex.forConditional(modelS0, fcastSpan);

condDataTbl{datex("2015-Q4"), "DOM_GDP"} = -1.5;
condDataTbl{datex("2016-Q4"), "DOM_CPI"} = 5.5;
condDataTbl{datex("2016-Q3"), "STN"} = 5.1;
condDataTbl{:, "Oil"} = inputTbl{end, "Oil"};

planTbl{datex("2015-Q4"), "DOM_GDP"} = "DEM POL";
planTbl{datex("2016-Q4"), "DOM_CPI"} = "DEM SUP";
planTbl{datex("2016-Q3"), "STN"} = "POL";
...
% NOTE: across-the-board variants (plan=[]) trigger a BEAR-internal bug
% in +conditional/forecast.m line 22 (cellfun on a non-cell []). Only the
% selective variant (plan=planTbl) is exercised here until BEAR is patched.
%
