...
% PATCH: model.Meta does not inherit from base.Meta, so base.Estimator.initialize
% rejects it. Switch the whole tutorial to the base.* namespace (kwargs renamed:
% endogenous->endogenousNames, exogenous->exogenousNames, shockConcepts->shockNames,
% units kwarg dropped — base.Meta is single-unit).
meta = base.Meta( ...
...
estimatorR1 = base.estimator.BetaTV(meta, stabilityThreshold=0.9999);
...
estimatorR2 = base.estimator.GeneralTV(meta);
...
modelS1 = base.Structural(reducedForm=modelR1, identifier=identChol);
modelS1.initialize();
...
modelS2 = base.Structural(reducedForm=modelR2, identifier=identChol);
modelS2.initialize();
