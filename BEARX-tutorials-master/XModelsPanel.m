...
dataH = cross.DataHolder(meta, inputTbx);

%% No CrossSection
% Panel model (NormalWishartPanel)


estimatorR1 = cross.estimator.StaticCrossPanel(meta);

modelR1 = cross.ReducedForm( ...
    meta=meta ...
    , dataHolder=dataH ...
...
modelS1 = cross.Structural(reducedForm=modelR1, identifier=identChol);
modelS1.initialize();
...
figure();
hold on
% PATCH: tablex.plot hard-codes plotFunc=@plot, so we call drawChart directly
% to override with @bar (BEAR-side wrapper limitation, see +tablex/plot.m).
tablex.drawChart(@bar, medianContShocks, "US_YER");
