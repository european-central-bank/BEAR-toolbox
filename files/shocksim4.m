function eta=shocksim4(cfconds,cfshocks,cfblocks,Fperiods,N,n,gamma_iter,fmat,ortirfcell)
...
% if there are no conditions, just draw shocks from their distributions
   if nblocks==0
   eta_t=mvnrnd(zeros(N*n,1),gamma_iter(:,:,ii))';
...
% if there are conditions
      ...
      for jj=1:nblocks
         ...
         for kk=1:N*n
            ...
            % loop over periods up to the one on which there is the constraint
               for ll=1:ii
               temp(1,(ll-1)*(N*n)+1:ll*(N*n))=ortirfcell{ii-ll+1,ii}(kk,:);
               end
            R(count,1:ii*(N*n))=temp;
            clear temp;
   ...
   % step 5: check if there are any non-constructive shocks
      ...
      temp=mvnrnd(zeros(N*n,1),gamma_iter(:,:,ii))';
      % update eta_t
