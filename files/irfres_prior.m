...
parfor ii=1:Acc %parfor
    ...
    if IRFt==4 && zerores==1 && signres==0 && magnres==0 && relmagnres==0 && favar_signres==0 && favar_magnres==0
        ...
        [~,ortirfmatrix]=irfsim_prior(beta,hsigma,n,m,p,k,max(IRFperiods,max(periodsmax)),prior);
        ...
        while success==0
            ...
            if size(Qj,2)==n && success==1  && relmagnres==1 | favar_relmagnres==1
                D=hsigma*Qj;
                [~,ortirfmatrixmagnitude]=irfsim_prior(beta,D,n,m,p,k,max(IRFperiods,max(mperiods)),prior);
            ...
            if size(Qj,2)==n && success==1  && FEVDres==1 | favar_FEVDres==1
                ...
                [~,ortirfmatrixFEVD]=irfsim_prior(beta,D,n,m,p,k,IRFperiods,prior); %obtain orthogonalized IRFs
                gamma=eye(n,n);
            ...
            if size(Qj,2)==n && success==1 && HD==1 && prior~=61
                ...
                if onestep==1 % her we use Xg and Yg
                    [hd_estimates]=hd_new_for_signres(const,exo,beta,k,n,p,D,m,T,Xg,Yg,data_exo,IRFt,signreslabels_shocks);
                else
                    [hd_estimates]=hd_new_for_signres(const,exo,beta,k,n,p,D,m,T,X,Y,data_exo,IRFt,signreslabels_shocks);
                end
            end
