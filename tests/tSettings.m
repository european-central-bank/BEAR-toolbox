classdef tSettings < matlab.unittest.TestCase
    ...
    methods(Test, TestTags = {'Git'})
        ...
        function tStrcident(tc)
            ...
            opts.IRFt = 5;
            opts.strctident.Instrument = 'test';
            opts.IRFt = 6;
            tc.verifyEqual(opts.strctident.Instrument, 'test');
            opts.IRFt = 4;
            opts.IRFt = 6;
            tc.verifyEqual(opts.strctident.Instrument, 'MHF');

            opts = BEARsettings(1,'ExcelFile','data.xlsx');
            tc.verifyEmpty(opts.strctident)
            opts.IRFt = 4;
            opts.strctident.CorrelShock = 'test';
            opts.IRFt = 6;
            tc.verifyEqual(opts.strctident.CorrelShock, 'test');

            opts = BEARsettings(5,'ExcelFile','data.xlsx');
            tc.verifyEmpty(opts.strctident)
            opts.IRFt = 5;
            opts.strctident.enddateIV = '2006m7';
            opts.IRFt = 6;
            tc.verifyEqual(opts.strctident.enddateIV, '2006m7');
