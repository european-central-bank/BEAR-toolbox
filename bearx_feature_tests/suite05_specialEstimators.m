function records = suite05_specialEstimators(opts)
%SUITE05_SPECIALESTIMATORS  Threshold + MixedFrequency + MeanAdjusted (probe).
    ...
    rec = tutil_runCase("suite05_special", "Threshold", ...
        @() runThreshold(opts.numSamples));
    records = [records, rec];

    rec = tutil_runCase("suite05_special", "MixedFrequency", ...
        @() runMixed(opts.numSamples));
    records = [records, rec];

    rec = tutil_runCase("suite05_special", "MeanAdjusted", ...
        @() runMean(opts.numSamples), "NOT IN GUI");
    records = [records, rec];
end


function runThreshold(numSamples)
    import threshold.*

    csvPath = tutil_synthThreshold();
...
function runMixed(numSamples)
    import mixed.*

    csvPath = tutil_synthMixed();
    inputTbl = tablex.fromCsv(csvPath);
...
function runMean(numSamples)
    ...
    assert(~isempty(fcastTbl), "bearx:test:meanFcastEmpty", ...
        "MeanAdjusted: forecast returned empty");
end
