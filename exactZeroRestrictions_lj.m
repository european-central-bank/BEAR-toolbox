...
% add checks for exact zeros
% Zcell is the ouput consistent with the bear. It is not needed, and we can completely remove it. I included it for debugging and reusing bear functions.
% if status is 0, then there is an error in the exact zeros table and message Msg will contain the error message.
[status, Zcell, Msg]=checkZeroRestriction_lj(meta, instantZerosTbx);
if status == 0
    error(Msg);
end

% this is the example of using the bear function qzerores to draw a random matrix Q satisfying the zero restrictions
oneSigma = modelS0.Presampled{1}.sigma;
hsigma = chol(bear.nspd(oneSigma),'lower');
Q = bear.qzerores(meta.NumShocks,Zcell, hsigma);
D=hsigma*Q;
