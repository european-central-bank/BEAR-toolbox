...
function [fcastTbl, contribsTbl] = conditionalForecast(this, fcastSpan, options)
    ...
    % BEAR6-FIX: was gated on hasPlan, leaving cfconds=[] (double) when Plan was
    % empty but Conditions were provided. Downstream +conditional/forecast.m then
    % crashed in `cellfun(@(x) ~isempty(x), cfconds)` with "Input #2 expected to
    % be a cell array, was double instead". Mirror +model/@Structural which always
    % builds the three CF arrays (they degrade gracefully when Plan is empty).
    hasPlan = ~isempty(options.Plan);
    ...
    if numSeparableUnits > 1
        % BEAR6-FIX: reshape each CF array independently (cfshocks/cfblocks may
        % be empty when Plan is not provided).
        if ~isempty(cfconds)
    ...
    for i = 1 : numPresampled
        ...
        for unit = 1 : numSeparableUnits
            % BEAR6-FIX: was gated on hasPlan. Pull the unit slice whenever the
            % corresponding CF array is non-empty (cfconds can be present even
            % without a Plan). cfblocks is now built once above.
            if ~isempty(cfconds)
                internalOptions.cfconds = cfconds(:, :, unit);
            end
            if ~isempty(cfshocks)
                internalOptions.cfshocks = cfshocks(:, :, unit);
                % BEAR6-FIX: shock indices in cfshocks are GLOBAL (over all
                % units) because createShocksCF builds its dict from
                % meta.ShockNames. Bring them back to per-unit indices, mirroring
                % +model/@Structural/conditionalForecast.
                for k = 1 : numel(internalOptions.cfshocks)
                    internalOptions.cfshocks{k} = internalOptions.cfshocks{k} - (unit-1)*numShockConcepts;
                end
            end
            if ~isempty(cfblocks)
                % BEAR6-FIX (Bug 9): cfblocks was built globally before the
                % per-unit slice, so a slice like [0,0,2] leaves shocksim6
                % looking for block jj=1 that lives in another unit (loc=[]
                % -> cfshocks{ii,[]}=[] -> "0-element assignment"). Rebuild
                % per-unit from the already-sliced cfconds/cfshocks instead.
                if ~isempty(internalOptions.cfconds) && ~isempty(internalOptions.cfshocks)
                    internalOptions.cfblocks = conditional.createBlocksCF( ...
                        internalOptions.cfconds, internalOptions.cfshocks);
                else
            ...
            unitLongY = longY;
            if numSeparableUnits > 1
                unitLongY = system.extractUnitFromNumericArray(unitLongY, unit, EXTRA_DIM);
            end
            ...
            unitBeta = draw.beta;
            if numSeparableUnits > 1
                % BEAR6-FIX: was EXTRA_DIM_BUG = 2 with a "TODO: rearrange" comment.
                % Inspection of conditionalDrawer in +separable/+estimator/*Panel.m
                % shows draw.beta = wrap(repmat(sample.beta, 1, 1, numCountries), ...),
                % i.e. units already live in the 3rd dimension. All sibling sites
                % (forecast.m, ReducedForm.m, Verifiables.m, Structural.m, ~14 calls)
                % use EXTRACT_DIM = 3. Aligning here.
                EXTRACT_DIM = 3;
                unitBeta = system.extractUnitFromCellArray(unitBeta, unit, EXTRACT_DIM);
            end
            %
            %
            ...
            sampleInitY = [sampleInitY, unitInitY]; %#ok<AGROW>
