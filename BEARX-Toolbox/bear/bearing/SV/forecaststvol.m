function [forecast_record]=forecaststvol(data_endo_a,data_exo_p,It,Bu,beta_gibbs,F_gibbs,phi_gibbs,L_gibbs,gamma_g,sbar,Fstartlocation,Fperiods,n,p,k,const)
...
phi=phi_gibbs(ii,:)';
if size(gamma_g,1) > 1
   gamma=gamma_g(ii,:)'; %it is called gamma_gibbs
else
   gamma = gamma_g;
end
...
lambda=L_gibbs(Fstartlocation-1,:,ii)';
   ...
   % loop over variables
      ...
      lambda(kk,1)=g*lambda(kk,1)+phi(kk,1)^0.5*randn;
      end
   % obtain Lambda_t
