...
classdef CarrieroSVFAVAR ...
    ...
    methods
        ...
        function initializeSampler(this, meta, longYX)
            ...
            % PATCH (BEARX-PATCHED): pad endo-sized settings to augmented numY
            % (numEndo + numFactors); see MinnesotaFAVARTwostep.m for rationale.
            nPad = numY - size(priorexo, 1);
            if nPad > 0
                priorexo    = [priorexo;    false(nPad, size(priorexo, 2))];
                opt.lambda4 = [opt.lambda4; repmat(opt.lambda4(end, :), nPad, 1)];
                opt.ar      = [opt.ar;      zeros(nPad, 1)];
            end
