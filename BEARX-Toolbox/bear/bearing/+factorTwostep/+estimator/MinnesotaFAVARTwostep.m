...
classdef MinnesotaFAVARTwostep ...
    ...
    methods
        function initializeSampler(this, meta, longYX)
            ...
            % PATCH (BEARX-PATCHED): Settings.Exogenous / Lambda4 / Autoregression
            % are pre-sized for NumEndogenousNames by EstimatorSettings.update,
            % but the prior helper below indexes them up to numEn = numEndo +
            % numFactors. Pad with the statistically correct defaults for the
            % standardized PCA factors (Exogenous=false, AR=0, Lambda4 replicated).
            nPad = numEn - size(priorexo, 1);
            if nPad > 0
                priorexo    = [priorexo;    false(nPad, size(priorexo, 2))];
                opt.lambda4 = [opt.lambda4; repmat(opt.lambda4(end, :), nPad, 1)];
                ar          = [ar;          zeros(nPad, 1)];
            end
