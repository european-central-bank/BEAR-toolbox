...
if favar.FAVAR==1
    ...
    if favar_signres==1 | favar_magnres==1
        ...
        relLindex=[favar.signresX_index;favar.plotX_index];
        npltX=favar.npltX;
        %signresX_index=favar.signresX_index;
        L=favar.L(relLindex,:);
        ...
        trnsfrm_endo=cell2mat(favar.trnsfrm_endo);
        variablestrings_exfactors=favar.variablestrings_exfactors;
        levels=favar.levels;
    else % provide NaNs
        favar_Scell=NaN;
        favar_Zcell=NaN;
...
for ii=1:Acc %parfor
    ...
    if IRFt==4 && zerores==1 && signres==0 && magnres==0 && relmagnres==0 && favar_signres==0 && favar_magnres==0
        ...
        [~,ortirfmatrix]=bear.irfsim_prior(beta,hsigma,n,m,p,k,max(IRFperiods,max(periodsmax)),prior);
        ...
        while success==0
            ...
            if size(Qj,2)==n && success==1  && relmagnres==1 | favar_relmagnres==1
                D=hsigma*Qj;
                [~,ortirfmatrixmagnitude]=bear.irfsim_prior(beta,D,n,m,p,k,max(IRFperiods,max(mperiods)),prior);
            ...
            if size(Qj,2)==n && success==1  && FEVDres==1 | favar_FEVDres==1
                ...
                [~,ortirfmatrixFEVD]=bear.irfsim_prior(beta,D,n,m,p,k,IRFperiods,prior); %obtain orthogonalized IRFs
                gamma=eye(n,n);
    ...
    if onestep==1
       for jj=1:IRFperiods
        favar_storage1{ii,1}(:,:,jj)=favar_ortirfmatrix(end-npltX+1:end,:,jj)*Q; % already loaded and transformed
       end
    end
    % store D
    storage2{ii,1}=D;
...
for ii=1:Acc % loop over iterations
    %for jj=1:IRFperiods % loop over IRF periods
        for kk=1:npltX % loop over variables
            for ll=1:n % loop over shocks
                favar.irf_record{kk,ll}(ii,:)=favar_storage1{ii,1}(kk,ll,:);
            end
        end
    %end
end
