...
for indj1=1:length(J1)
    ...
    [m0(counter:counter+equilibrium(j)-1),C0(counter:counter+equilibrium(j)-1,counter:counter+equilibrium(j)-1)]=bear.OLSPriorTheta(data_endo(:,j),timematrix(:,1:equilibrium(j)),f);
    end
...
for i=1:NumberofRegimes
    for indjr=1:length(Jr)
        ...
        [m0(counter:counter+equilibrium(j)-1),C0(counter:counter+equilibrium(j)-1,counter:counter+equilibrium(j)-1)]=bear.OLSPriorTheta(datatemp,timetemp,f);    
        end;
        counter=counter+equilibrium(j);
    end
end
