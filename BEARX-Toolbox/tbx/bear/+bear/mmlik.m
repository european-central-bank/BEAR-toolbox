...
[logml log10ml ml]=bear.mmlikdata(X,y,n,T,q,sigma,beta0,omega0,betabar);

% if there are dummy observations in the data set
if (scoeff==1 || iobs==1)
% then compute posterior elements for the dummy observations
[betabardum,~]=bear.mpost(beta0,omega0,sigma,Xdum,ydum,q,n);
% and use those elements to compute the marginal likelihood for the dummy observations only
[logmldum log10mldum mldum]=bear.mmlikdata(Xdum,ydum,n,Tdum,q,sigma,beta0,omega0,betabardum);
