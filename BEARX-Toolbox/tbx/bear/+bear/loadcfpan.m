...
[~, ~, strngs]=xlsread(pref.excelFile,'pan conditions');
...
if isempty(temp)
    ...
    error('bear:loadcfpan:FstartdateNotFound',message)
end
% otherwise, record
rows(1,1)=temp;
...
if isempty(temp)
    ...
    error('bear:loadcfpan:FenddateNotFound',message)
end
% otherwise, record
rows(2,1)=temp;
    ...
    for ii=1:numunits
        ...
        for jj=1:numendo
            ...
            if isempty(clmns)
                ...
                error('bear:loadcfpan:endoNotFound',message)
            end
...
if CFt==2
    % load the data from Excel
    [~, ~, strngs]=xlsread(pref.excelFile,'pan shocks');
    % replace NaN entries by blanks
    strngs(cellfun(@(x) any(isnan(x)),strngs))={[]};
    % convert potential numeric entries into strings
    strngs(cellfun(@isnumeric,strngs))=cellfun(@num2str,strngs(cellfun(@isnumeric,strngs)),'UniformOutput',0);
...
if CFt==2
    % load the data from Excel
    [~, ~, strngs]=xlsread(pref.excelFile,'pan blocks');
    % replace NaN entries by blanks
    strngs(cellfun(@(x) any(isnan(x)),strngs))={[]};
    % convert potential numeric entries into strings
    strngs(cellfun(@isnumeric,strngs))=cellfun(@num2str,strngs(cellfun(@isnumeric,strngs)),'UniformOutput',0);
    ...
    if CFt==2
        ...
        for ii=1:numunits
            ...
            if nconds==0
                ...
                [nerows3, ~]=find(~cellfun('isempty',temp));
                % if the number of non-empty elements is not similar in the two cells, there is obvioulsy a problem: return an error
