...
parfor ii=1:Acc %parfor
    ...
    if IRFt==4 && zerores==1 && signres==0 && magnres==0 && relmagnres==0 && favar_signres==0 && favar_magnres==0
        ...
        [~,ortirfmatrix]=bear.irfsim(beta,hsigma,n,m,p,k,max(IRFperiods,max(periodsmax)));
        ...
        while success==0
            ...
            if (transformation==1 || plot_transform==1) && favar_retransres==1
                [ortirfmatrix]=bear.favar_retransX_ortirfmatrix(ortirfmatrix,transformationindex_endo,levels);
            end
            ...
            if npltX+nsignresX~=0%favar_signres==1 | favar_magnres==1
                ...
                % approx. transform back here, as otherwise we would restrict the transformed variables
                if (transformation==1 || plot_transform==1) && favar_retransres==1
                    [favar_ortirfmatrix]=bear.favar_retransX_ortirfmatrix(favar_ortirfmatrix,transformationindex,levels);
                end
            ...
            if size(Qj,2)==n && success==1  && FEVDres==1 | favar_FEVDres==1
                ...
                % approx. transform back here, as otherwise we would restrict the transformed variables
                if (transformation==1 || plot_transform==1) && favar_retransres==1
                    [ortirfmatrixFEVD]=bear.favar_retransX_ortirfmatrix(ortirfmatrixFEVD,transformationindex_endo,levels);
                end
                ...
                if success==1 && favar_FEVDres==1
                    ...
                    % approx. transform back here, as otherwise we would restrict the transformed variables
                    if (transformation==1 || plot_transform==1) && favar_retransres==1
                        [favar_ortirfmatrixFEVD]=bear.favar_retransX_ortirfmatrix(favar_ortirfmatrixFEVD,transformationindex,levels);
                    end
                    % check
