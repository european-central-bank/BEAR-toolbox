function [irf_record,D_record,gamma_record,hd_record,ETA_record,beta_record,sigma_record]=irfres_stvol4(beta_gibbs,sigma_gibbs,C_draws,IV_draws,IRFperiods,n,m,p,k,T,Y,X,signreslabels,FEVDresperiods,data_exo,HD,const,exo,strctident,pref,favar,IRFt,It,Bu,YincLags, Psi_gibbs, sizetraining)
    ...
    if IRFt==4 && zerores==1 && signres==0 && magnres==0 && relmagnres==0 && favar_signres==0 && favar_magnres==0
        ...
        while success==0
            ...
            if size(Qj,2)==n && success==1  && checkCorrelInstrumentShock==1
                ...
                ETA=D\EPS';
                [success,corIV,pivShockwithIV,Qj]=bear.CheckCorrelWithIV(ETA,n,CorrelShock_index, IVcorrel,OverlapIVcorrelinY(sizetraining+1:end,:),FlipCorrel,Qj);
%                 if success==1
...
%                   end 
            ...
            if size(Qj,2)==n && success==1 && HD==1
                ...
                [hd_estimates] = bear.hd_for_signres_stvol4(const,exo,beta,k,n,p,D,m,T,Xdraw,Ydraw,data_exo,IRFt,signreslabels_shocks,Psidrawcut, YincLags(2*p+1:end,:));
            end
