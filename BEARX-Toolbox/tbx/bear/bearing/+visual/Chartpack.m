        ...
        function [figureHandles, plotHandles] = plot(this, table)
            ...
            for f = 1 : this.NumFigures
                ...
                for p = 1 : numel(this.NamesToPlot{f})
                    ...
                    % BEAR6-FIX: original code passed "plotFunc" and "barStyle" kwargs
                    % to tablex.plot, but tablex.plot/tablex.drawChart do not accept
                    % those kwargs, causing MATLAB:TooManyInputs on every plot() call.
                    % Route through tablex.drawChart for non-default PlotFunc, and
                    % fold BarStyle into PlotSettings for bar charts.
                    plotSettingsLocal = this.PlotSettings;
                    if isequal(this.PlotFunc, @bar) && ~isempty(this.BarStyle)
                        plotSettingsLocal = [plotSettingsLocal, {"BarLayout", this.BarStyle}];
                    end
                    if isequal(this.PlotFunc, @plot)
                        h = tablex.plot( ...
                            table, nameToPlot, ...
                            "periods", this.Span, ...
                            "axesHandle", ax, ...
                            "plotSettings", plotSettingsLocal ...
                        );
                    else
                        h = tablex.drawChart( ...
                            this.PlotFunc, table, nameToPlot, ...
                            "periods", this.Span, ...
                            "axesHandle", ax, ...
                            "plotSettings", plotSettingsLocal ...
                        );
